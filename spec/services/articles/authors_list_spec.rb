...
describe Articles::AuthorsList do
  ...
  let(:four_author_doc) { documents.first }
  let(:three_author_doc) { documents.second }
  let(:two_author_doc) { documents.last }
  ...
  describe '.new' do
    it 'returns an Articles::AuthorsList object' do
      expect(four_author_doc.authors).to be_a(Articles::AuthorsList)
    end

    it 'builds an authors list' do
      expect(four_author_doc.authors.list).to be_a(String)
    end
  end
  ...
  describe '#list' do
    context 'when there are two authors' do
      it 'returns author names in first last format separated by "and"' do
        expect(two_author_doc.authors.list).to eq('Rebecca Schmid and John Doe')
      end
    end

    context 'when there are three authors' do
      it 'returns a comma-separated list of author names in first last format' do
        expect(three_author_doc.authors.list).to eq('René L Schilling, Renming Song, and Zoran Vondracek')
      end
    end

    context 'when there are four authors' do
      it 'returns first two author names in first last format followed by "et al."' do
        expect(four_author_doc.authors.list).to eq('Martin Fischer, Howard W Ashcraft, et al.')
      end
    end
  end
end
