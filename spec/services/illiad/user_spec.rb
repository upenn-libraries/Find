...
describe Illiad::User do
  ...
  describe '.find' do
    context 'with a successful request' do
      before { stub_find_user_success(id: illiad_user.id, response_body: build(:illiad_user_response)) }
    ...
    context 'with an unsuccessful request' do
      before { stub_find_user_failure(id: illiad_user.id, response_body: build(:illiad_api_error_response)) }
  ...
  describe '.create' do
    ...
    let(:request_body) { { UserName: username } }

    context 'with a successful request' do
      let(:response_body) { build(:illiad_user_response, UserName: username) }
  ...
  describe '.requests' do
    ...
    context 'with a successful request' do
      let(:response_body) { build_list(:illiad_api_request_response, 2, :loan) }
