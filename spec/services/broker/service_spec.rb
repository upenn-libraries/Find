...
describe Broker::Service do
  ...
  describe '#submit' do
    context 'with a submission destined for illiad' do
      ...
      context 'with an invalid request' do
        ...
        it 'returns a failed outcome' do
          expect(result).to be_a Broker::Outcome
    ...
    context 'with a submission destined for Alma' do
      let(:backend) { Broker::Backend::Alma }
      let(:request) { build(:broker_request, :with_item, :pickup) }

      context 'with a valid request' do
        let(:validation_errors) { [] }
        let(:outcome) { Broker::Outcome.new(request: request, confirmation_number: 'ALMA1234') }
      ...
      context 'with an invalid request' do
        let(:validation_errors) { ['Some required value not set!'] }
        let(:outcome) { Broker::Outcome.new(request: request, errors: validation_errors) }
