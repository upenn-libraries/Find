...
FactoryBot.define do
  factory :library_info_request_response, class: 'Library::Info::Request' do
    sequence(:num) { |n| n }
    add_attribute(:name) { 'The Library' }
    add_attribute(:node_id) { Faker::Number.number(digits: 4) }
    ...
    trait :with_all_info do
      ...
      add_attribute(:address1) { Faker::Address.street_address }
      add_attribute(:address2) { Faker::Address.secondary_address }
      ...
      add_attribute(:libcal_id) { Faker::Number.number(digits: 3) }
